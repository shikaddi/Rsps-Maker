package main.java.org.necrotic.client.world;

import main.java.org.necrotic.Configuration;
import main.java.org.necrotic.client.Class21;
import main.java.org.necrotic.client.Class33;
import main.java.org.necrotic.client.Client;
import main.java.org.necrotic.client.FrameReader;
import main.java.org.necrotic.client.SkinList;
import main.java.org.necrotic.client.cache.ondemand.OnDemandFetcherParent;
import main.java.org.necrotic.client.graphics.DrawingArea;
import main.java.org.necrotic.client.io.ByteBuffer;
import main.java.org.necrotic.client.renderable.Animable;
...
public class Model extends Animable {
	...
	private void read525Model(byte abyte0[], int modelID) {
		System.out.println("model 525 :" + modelID);
		...
		for (int k11 = 0; k11 < numVertices; k11++) {
			...
			if (modelID == 1044 && k11 < 8) {
			System.out.printf("JAVA 525 V%d: (%d,%d,%d)%n", k11, vertexX[k11], vertexY[k11], vertexZ[k11]);
			}
		}
		nc1.position = j8;
		nc2.position = i6;
        ...
        if (modelID == 1044 && i14 < 8) {
            System.out.printf("JAVA 525 F%d: (%d,%d,%d)%n", i14, facePoint1[i14], facePoint2[i14], facePoint3[i14]);
        }
		}
		nc1.position = j9;
		nc2.position = k9;
		...
		if (modelID == 1044) {
			System.out.println("JAVA 525 modelId=" + modelID);
			System.out.println("v=" + numVertices + " f=" + numTriangles + " t=" + numTextureTriangles);
			// Offsets and lengths mirroring Python logs
			System.out.printf("offs[v_flags] = off=%d len=%d%n", l5, numVertices);
			System.out.printf("offs[vert_x] = off=%d len=%d%n", k8, j3);
			System.out.printf("offs[vert_y] = off=%d len=%d%n", l8, k3);
			System.out.printf("offs[vert_z] = off=%d len=%d%n", i9, l3);
			System.out.printf("offs[tri_types] = off=%d len=%d%n", j6, numTriangles);
			System.out.printf("offs[face_idx16] = off=%d len=%d%n", k7, i4);
			System.out.printf("offs[colours] = off=%d len=%d%n", j8, numTriangles * 2);
			int texLen = (l2 == 1 ? numTriangles * 2 : 0);
			System.out.printf("offs[tex_idx] = off=%d len=%d%n", l7, texLen);

			// Hex previews
			System.out.printf("tri_types[0:32]= %s%n", toHex(abyte0, j6, Math.min(32, numTriangles)));
			System.out.printf(" face_idx16[0:32]= %s%n", toHex(abyte0, k7, Math.min(32, i4)));

			// Flags demand vs lengths
			int needX = 0, needY = 0, needZ = 0;
			for (int i = 0; i < numVertices; i++) {
				int fl = abyte0[l5 + i] & 0xFF;
				if ((fl & 1) != 0) needX++;
				if ((fl & 2) != 0) needY++;
				if ((fl & 4) != 0) needZ++;
			}
			System.out.printf("needX/Y/Z=%d/%d/%d lenX/Y/Z=%d/%d/%d%n", needX, needY, needZ, j3, k3, l3);
		}
	}

	// Helper to preview bytes in hex for quick trailer/stream verification
		private static String toHex(byte[] arr, int off, int len) {
			StringBuilder sb = new StringBuilder();
			int n = Math.max(0, Math.min(len, arr.length - off));
			for (int i = 0; i < n; i++) {
				sb.append(String.format("%02x", arr[off + i]));
			}
			return sb.toString();
		}
	
	private void read622Model(byte abyte0[], int modelID) {
		System.out.println("model 622 :" + modelID);
		ByteBuffer nc1 = new ByteBuffer(abyte0);
		ByteBuffer nc2 = new ByteBuffer(abyte0);
		ByteBuffer nc3 = new ByteBuffer(abyte0);
		ByteBuffer nc4 = new ByteBuffer(abyte0);
		...
		translate(0, 6, 0);
		if (modelID == 1044) {
			System.out.println("JAVA 622 modelId=" + modelID);
			System.out.println("v=" + numVertices + " f=" + numTriangles + " t=" + numTexTriangles + " newfmt=" + newformat);
			// Offsets and lengths mirroring Python logs
			System.out.printf("offs[v_flags] = off=%d len=%d%n", l5, numVertices);
			System.out.printf("offs[vert_x] = off=%d len=%d%n", k8, j3);
			System.out.printf("offs[vert_y] = off=%d len=%d%n", l8, k3);
	...
	private void readOldModel(int i) {
		System.out.println("model 317 :" + i);
		int j = -870;
		aBoolean1618 = true;
		aBoolean1659 = false;
		Class21 class21 = aClass21Array1661[i];
		...
		// Debug: dump trailer/offsets for specific model id
				if (i == 1044) {
					Class21 c = aClass21Array1661[i];
					int v = c.anInt369, f = c.anInt370, t = c.anInt371;
					System.out.println("JAVA 317 modelId=" + i);
					System.out.println("v=" + v + " f=" + f + " t=" + t);
					System.out.printf("offs[v_flags] = off=%d len=%d%n", c.anInt372, v);
					// Derive axis lengths from consecutive offsets
					int lenX = c.anInt374 - c.anInt373;
					int lenY = c.anInt375 - c.anInt374;
					int lenZ = c.aByteArray368.length - c.anInt375;
					System.out.printf("offs[vert_x] = off=%d len=%d%n", c.anInt373, lenX);
					System.out.printf("offs[vert_y] = off=%d len=%d%n", c.anInt374, lenY);
					System.out.printf("offs[vert_z] = off=%d len=%d%n", c.anInt375, lenZ);
					System.out.printf("offs[tri_types] = off=%d len=%d%n", c.anInt378, f);
					int lenCmd = c.anInt379 - c.anInt377;
					System.out.printf("offs[tri_cmds] = off=%d len=%d%n", c.anInt377, lenCmd);
					System.out.printf("offs[colours] = off=%d len=%d%n", c.anInt379, f * 2);
					System.out.printf("tri_types[0:32]= %s%n", toHex(c.aByteArray368, c.anInt378, Math.min(32, f)));
					System.out.printf("  tri_cmds[0:32]= %s%n", toHex(c.aByteArray368, c.anInt377, Math.min(32, lenCmd)));
					int needX = 0, needY = 0, needZ = 0;
					for (int vi = 0; vi < v; vi++) {
						int fl = c.aByteArray368[c.anInt372 + vi] & 0xFF;
						if ((fl & 1) != 0) needX++;
						if ((fl & 2) != 0) needY++;
						if ((fl & 4) != 0) needZ++;
					}
					System.out.printf("needX/Y/Z=%d/%d/%d lenX/Y/Z=%d/%d/%d%n", needX, needY, needZ, lenX, lenY, lenZ);
				}
